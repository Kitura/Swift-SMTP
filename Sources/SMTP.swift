...
public class SMTP {
    ...
    /**
     ...
     - Each call to `send` queues it's `Mail`s and sends them one by one. To
     send `Mail`s concurrently, send them in separate calls to `send`.
     */
    public func send(_ mails: [Mail], progress: Progress = nil, completion: Completion = nil) {
        do {
            let socket = try SMTPLogin(hostname: hostname, user: user, password: password, accessToken: accessToken, domainName: domainName, authMethods: validAuthMethods, chainFilePath: chainFilePath, chainFilePassword: chainFilePassword, selfSignedCerts: selfSignedCerts).login()
...
public extension SMTP {
    ...
    var validAuthMethods: [AuthMethod] {
